...
export default function DatesPage() {
  ...
  const [bulkPreview, setBulkPreview] = useState<{ date: string; prices: number[] }[] | null>(null);
  ...
  const parseBulkText = () => {
    ...
    if (lines.length === 0) {
      setBulkError('No hay datos para procesar');
      return;
    }
    ...
    if (isHeader) {
      // Skip col 0 (date) and col 1 (category); the rest are price columns
      for (let i = 2; i < firstCells.length; i++) {
        if (firstCells[i]) headers.push(firstCells[i]);
      }
    } else {
      for (let i = 2; i < firstCells.length; i++) {
        headers.push(`Precio ${i - 1}`);
      }
    }
    ...
    for (const line of dataLines) {
      ...
      for (let i = 2; i < cells.length; i++) {
        const price = parseFloat((cells[i] || '').replace(/,/g, ''));
    ...
    if (rows.length === 0) {
      setBulkError('No se encontraron fechas válidas. Asegúrate de que las fechas estén en formato M/D/AAAA');
      return;
    }
    ...
    // Auto-match price columns to packages by name similarity
    const norm = (s: string) =>
      s.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    const autoMapping: Record<number, string> = {};
    headers.forEach((header, idx) => {
      const normalHeader = norm(header);
      const match = packages.find(pkg => {
        const normalPkg = norm(pkg.name);
        return normalPkg.includes(normalHeader) || normalHeader.includes(normalPkg);
      });
      if (match) autoMapping[idx] = match.id;
    });
  ...
  const applyBulkImport = () => {
    ...
    const newDates: LocalTourDate[] = bulkPreview.map(row => {
      ...
      return {
        ...
        package_overrides: packages.map(pkg => {
          const colEntry = Object.entries(columnMapping).find(([, pkgId]) => pkgId === pkg.id);
          const price = colEntry !== undefined ? row.prices[Number(colEntry[0])] : null;
          return {
            package_id: pkg.id,
            enabled: true,
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="mb-4 flex gap-2">
        ...
        <Button
          variant="outline"
          onClick={() => { setShowBulkPaste(!showBulkPaste); setBulkPreview(null); setBulkError(null); }}
      ...
      {/* Bulk Paste Panel */}
      {showBulkPaste && (
        <Card className="mb-6 border-[#9996DB]">
          <CardHeader className="pb-3">
            <CardTitle className="text-base text-[#3546A6]">Pegar fechas en bulk</CardTitle>
            <CardDescription>
              Copia y pega directamente desde Excel. Columnas esperadas: <strong>Fecha · Categoría · Precio 1 · Precio 2 · Precio 3</strong>
            </CardDescription>
          </CardHeader>
          <CardContent>
            {!bulkPreview ? (
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" onClick={resetBulkPaste}>
                    Cancelar
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-5">
                ...
                <div>
                  <p className="text-sm font-medium text-slate-700 mb-2">
                    {bulkPreview.length} fechas detectadas
                  </p>
                    ...
                    <table className="text-xs w-full">
                      <thead className="bg-slate-100 sticky top-0">
                        <tr>
                          ...
                          {bulkHeaders.map((h, i) => (
                            <th key={i} className="text-left p-2 font-medium text-slate-600">{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {bulkPreview.slice(0, 6).map((row, i) => (
                          <tr key={i} className="border-t border-slate-100">
                            <td className="p-2 text-slate-700">{formatDate(row.date)}</td>
                            {row.prices.map((p, j) => (
                              <td key={j} className="p-2 text-slate-700">${p.toLocaleString()}</td>
                            ))}
                          </tr>
                        ))}
                        {bulkPreview.length > 6 && (
                          <tr className="border-t border-slate-100">
                            <td colSpan={bulkHeaders.length + 1} className="p-2 text-slate-400 text-center italic">
                              ... y {bulkPreview.length - 6} fechas más
                            </td>
                          </tr>
                        )}
                      </tbody>
                ...
                {/* Column → Package mapping */}
                {packages.length > 0 && bulkHeaders.length > 0 && (
                  <div>
                    <p className="text-sm font-medium text-slate-700 mb-2">
                      Asignar columnas de precio a paquetes
                    </p>
                    <div className="space-y-2">
                      {bulkHeaders.map((header, idx) => (
                        <div key={idx} className="flex items-center gap-3">
                          ...
                          <Select
                            value={columnMapping[idx] || 'unassigned'}
                            onValueChange={(val) =>
                              setColumnMapping(prev => ({
                                ...prev,
                                [idx]: val === 'unassigned' ? '' : val,
                              }))
                            }
                          >
                    ...
                    {packages.length === 0 && (
                      <p className="text-xs text-amber-600 mt-2">
                        Sin paquetes configurados — los precios quedarán sin asignar. Puedes editarlos después.
                      </p>
                    )}
                  </div>
                )}
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  >
                    <Plus className="w-4 h-4 mr-2" />
                    Importar {bulkPreview.length} fechas
                  </Button>
                                    ...
                                    <div className="flex items-center justify-between p-3 cursor-pointer hover:bg-slate-50 transition-colors">
                                      <div className="flex items-center gap-3">
                                        ...
                                        {override.price_override !== null && (
                                          <Badge variant="outline" className="text-xs text-[#3546A6]">
                                            ${override.price_override.toLocaleString()}
                                          </Badge>
                                        )}
                                        {pkg.is_default && (
        ...
        {dates.length === 0 && (
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-sm text-slate-400 mt-1">Agrega una fecha o usa "Pegar en bulk" para importar desde Excel</p>
            </CardContent>
          </Card>
        )}
      </div>
